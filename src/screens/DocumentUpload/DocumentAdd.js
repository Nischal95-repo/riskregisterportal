...
class DocumentAdd extends React.Component {
  ...
  submitModal() {
    this.setState({ reactModalVisible: false, modalMessage: "" }, () => {
      // redirectTo("/upload-document-detail?id="+this.state.documentId);
      this.props.history.push(
        "/upload-document-detail?id=" + this.state.documentId+"&create=true"
      );
    });
  }
      ...
      this.props.client
        ...
        .then(result => {
          this.setState({
            ...
            modalMessage: "Task Created Successfullly"
          });
        })
        .catch(error => {
  ...
  render() {
    ...
    return (
      <>
        ...
        />
        <h1 className="heading m-b-25">Task Creation</h1>
        <div
          className="row"
          style={{
