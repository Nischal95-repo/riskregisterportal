...
export const nameValidation = name => {
  ...
  const FORMAT_REGEX = new RegExp("^[A-Za-z]([a-zA-Z0-9-_]|[" + NAME_VALIDATION + "])*$");
...
export const alphabetAcceptanceValidation = (name, format) => {
  ...
  const FORMAT_REGEX = new RegExp("^[A-Za-z]([a-zA-Z]|[" + format1 + "])*$");
  console.log("regex", FORMAT_REGEX);
  if (FORMAT_REGEX.test(name)) {
    valid = true;
  }
...
export const alphanumericAcceptanceValidation = (name, format) => {
  let valid = false;
  let format1 = format ? format : "";
  const FORMAT_REGEX = new RegExp("^[A-Za-z]([a-zA-Z0-9_]|[" + format1 + "])*$");
  console.log("regex", FORMAT_REGEX);
  if (FORMAT_REGEX.test(name)) {
    valid = true;
  }
...
export const alphanumericOnlyValidation = (name, format) => {
  let valid = false;
  let format1 = format ? format : "";
  const FORMAT_REGEX = new RegExp("^([a-zA-Z0-9]|[" + format1 + "])*$");
  console.log("regex", FORMAT_REGEX);
  if (FORMAT_REGEX.test(name)) {
    valid = true;
  }
