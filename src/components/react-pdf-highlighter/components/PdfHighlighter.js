...
type Props<T_HT> = {
  ...
  nextPageRef: (nextPage: () => void) => void,
  prevPageRef: (prevPage: () => void) => void,
  scrollToPageRef: (toPage: (pageNumber: any) => void) => void,
  zoomInRef: (zoomIn: () => void) => void,
  zoomOutRef: (zoomOut: () => void) => void,
  searchRef: (
    search: (
      text: any,
      matchCase: any,
  ...
  searchNextRef: (searchNext: () => void) => void,
  searchPrevRef: (
    search: (
      text: any,
      matchCase: any,
...
> {
  ...
  componentDidUpdate(prevProps: Props<T_HT>) {
    ...
    // if (this.state.currentPageNum != parseInt(this.viewer._currentPageNumber)) {
    //   console.log("update", this.viewer._currentPageNumber);
    //   this.setState(
    //     {
    //       currentPageNum: parseInt(this.viewer._currentPageNumber)
    //     }
    //     // () => {
    //     //   this.props.updatepageNumber(this.viewer._currentPageNumber);
    //     // }
    //   );
    // }
  }
  ...
  componentDidMount() {
    ...
    this.viewer.setFindController(this.findController);
  }
  ...
  nextPage = () => {
    ...
    // this.viewer.container.removeEventListener("scroll", this.onScroll);
    let currentPage = this.viewer._currentPageNumber;
    let totalPages = this.viewer.pdfDocument.pdfInfo.numPages;
    console.log(this.viewer, currentPage, totalPages);
  ...
  scrollToPage = pageNumber => {
    // let currentPage = this.viewer._currentPageNumber;
    let totalPages = this.viewer.pdfDocument.pdfInfo.numPages;
    console.log(totalPages, this.viewer);
  ...
  zoomIn = () => {
    ...
    this.viewer.currentScaleValue = parseFloat(currentScaleValue) + 0.25;
  };
  zoomOut = () => {
    let currentScaleValue =
      this.viewer.currentScaleValue == "auto"
  ...
  search = (text, matchCase, highlightAll, findPrevious) => {
    console.log("findPrevious !!!!!!!!!!!!!!", findPrevious);

    let type = "find";
    if (findPrevious == this.state.findPrevious) {
      type = "find";
    } else {
      type = "findagain";
    }

    if (this.state.previousHighlight != highlightAll) {
    ...
    console.log(
      "occurance in pdf 1",
      this.findController,
      this.findController.matchCount
    );
    this.setState({
      findPrevious: findPrevious,
      previousHighlight: highlightAll
    });
  ...
  searchNext = () => {
    ...
    if (this.findController.matchCount > 1) {
      this.findController._nextMatch();
    }
  };
  getPageNumber = () => {
    return this.viewer._currentPageNumber;
  };
  onDocumentReady = () => {
    const {
      scrollRef,
    ...
    nextPageRef(this.nextPage);
    prevPageRef(this.prevPage);
    scrollToPageRef(this.scrollToPage);
    zoomInRef(this.zoomIn);
    zoomOutRef(this.zoomOut);
    searchRef(this.search);
    searchNextRef(this.searchNext);
    searchPrevRef(this.search);
    getPageNumberRef(this.getPageNumber);
  };
