...
import { getDocumnetListQuery } from "../../services/graphql/queries/document-upload";
...
const DOCUMENT_UPLOAD_MODULE_ID = 2;
const DOCUMENT_UPLOAD_FEATURE_ID = 110;
...
class DocumentsList extends React.Component {
  ...
  onAddDocument() {
    // if (this.state.accessSpecifier.viewP) {
      // redirectTo("/document-add");
      this.props.history.push("/document-add");
    // } else {
  ...
  getListOfDocuments(offset, pageNo, data){
    this.props.client
      ...
      .then(result => {
        var document = result.data.getListOfDocuments;
  ...
  componentDidMount(){
    ...
    this.props.client
      .query({
        ...
        variables: {
          moduleId: DOCUMENT_UPLOAD_MODULE_ID
        },
        fetchPolicy: "network-only"
      })
      .then(result => {
        ...
        this.setState({
          accessSpecifier: response[DOCUMENT_UPLOAD_FEATURE_ID]
        });
          ...
          <div>
            ...
            <div className="row align-items-center no-gutters">
                <div className="col-md-8">
                    <h1 className="heading m-b-0">Task List </h1>
                </div>
                <div className="col-md-4 text-right">
                    <a href="#" className="btn btn-danger" onClick={() => this.onAddDocument()}>Add</a>
                          ...
                          pageOfItems.map((data,index)=>{
                            return(
                              <tr key={index}>
                                ...
                                <td>{format(data.lastModifiedOn, dateFormatMonth)}</td>
                                <td>{data.statusDetail.length ? data.statusDetail[0].name : ""}</td>
                                <td><a href={"/upload-document-detail?id="+data.Id} className="link-primary" data-toggle="tooltip" data-placement="bottom" title="View">
