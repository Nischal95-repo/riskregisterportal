...
export const errorMessage = (error, view) => {
  ...
  const DEFAULT_MESSAGE = "An error has occurred. Please contact your system administrator";

  const UPLOAD_ERROR_MESSAGE = "Cannot upload files more than " + MAX_UPLOAD_SIZE + " MB";
  ...
  if (error.graphQLErrors.length == 0) {
    ...
    if (error.message.toUpperCase().includes("NETWORK ERROR") && view === 1) {
      return "File cannot exceed " + MAX_UPLOAD_SIZE + " MB. Please contact your system administrator";
    }
  ...
  return error.graphQLErrors.map(({ message, extensions }) => {
    ...
    if (message.toUpperCase().includes("STATUS CODE 400")) {
      ...
      if (error.toUpperCase().includes("DATA_UPLOAD_MAX_MEMORY_SIZE")) return UPLOAD_ERROR_MESSAGE;
